...
ros_ign_interfaces__msg__WorldReset__create()
{
  ...
  ros_ign_interfaces__msg__WorldReset * msg = (ros_ign_interfaces__msg__WorldReset *)allocator.allocate(sizeof(ros_ign_interfaces__msg__WorldReset), allocator.state);
  if (!msg) {
    return NULL;
  }
...
ros_ign_interfaces__msg__WorldReset__Sequence__init(ros_ign_interfaces__msg__WorldReset__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (ros_ign_interfaces__msg__WorldReset *)allocator.zero_allocate(size, sizeof(ros_ign_interfaces__msg__WorldReset), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros_ign_interfaces__msg__WorldReset__Sequence * array = (ros_ign_interfaces__msg__WorldReset__Sequence *)allocator.allocate(sizeof(ros_ign_interfaces__msg__WorldReset__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  ...
  ros_ign_interfaces__msg__WorldReset__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ros_ign_interfaces__msg__WorldReset * data =
      (ros_ign_interfaces__msg__WorldReset *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros_ign_interfaces__msg__WorldReset__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros_ign_interfaces__msg__WorldReset__fini(&output->data[i]);
        }
        return false;
      }
    }
